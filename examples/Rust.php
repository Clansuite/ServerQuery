...
$config = [
    'server_addr'     => '216.39.240.160',
    'server_port'     => 28015, // query port
    'request_timeout' => 2,
];

$server = new Rust($config['server_addr'], $config['server_port']);

print "Querying Rust server {$config['server_addr']}:{$config['server_port']}...\n";
...
if ($server->query_server(true, true)) {  // enable players and rules
    ...
    if ($server->players !== []) {
        ...
        foreach (\array_slice($server->players, 0, 5) as $p) {  // show first 5
            ...
            $time  = \is_scalar($p['time'] ?? 0) ? (float) ($p['time'] ?? 0) : 0.0;
            \printf(" - %s (score: %d, time: %.1fs)\n", $name, $score, $time);
        }
