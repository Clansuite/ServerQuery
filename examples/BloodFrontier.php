...
// Blood Frontier server example
$serverAddress = '127.0.0.1'; // Replace with actual server IP
$queryPort     = 28763; // Default Blood Frontier game port (query uses port + 1)

print "Querying Blood Frontier server at {$serverAddress}:{$queryPort}..." . \PHP_EOL;

try {
    $server = $csQuery->createInstance('BloodFrontier', $serverAddress, $queryPort);
