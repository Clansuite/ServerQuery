...
// Cube 1 server example
$serverAddress = '127.0.0.1'; // Replace with actual server IP
$queryPort     = 28763; // Default Cube 1 game port (query uses port + 1)

print "Querying Cube 1 server at {$serverAddress}:{$queryPort}..." . \PHP_EOL;

try {
    $server = $csQuery->createInstance('Cube1', $serverAddress, $queryPort);
