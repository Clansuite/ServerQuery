...
final class EcoFixtureTest extends TestCase
{
    public function testFixtureExistsAndHasMetadata(): void
    {
        $dir = __DIR__ . '/../../../tests/fixtures/eco/v0_11_1_13_beta_release_887';

        $json = $dir . '/capture_46_4_156_247_3001.json';
        ...
        $this->assertNotEmpty($serverInfo['mapname'], 'Map name should not be empty');
        $this->assertGreaterThan(0, $serverInfo['numplayers'], 'Player count should be greater than 0');
        $this->assertGreaterThan(0, $serverInfo['maxplayers'], 'Max players should be greater than 0');
        $this->assertNotEmpty($serverInfo['players'], 'Player list should not be empty');
        $this->assertNotEmpty($serverInfo['rules'], 'Server rules should not be empty');
    }
}
